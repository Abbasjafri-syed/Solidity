...
22.	Forge test not allowed passing args in cl.
23.	Testing needs to be developed around 3 A’s i.e. Arrange, Act and Assert.
24.	Arrange is setting all parameters and initial state of all variables...Act is calling the contract or method in contract...assert is validating state changes taken place in Act.
25.	Branching tree Technique or BTT is method for organising unit testing based on state changing taken place at each step.
26.	BTT starts with default behaviour of contract and then modifier are created and added before each state changing.
27.	 ‘vm’ cheat code ignore the next line if also the ‘vm’ is present.
28.	 makeAddr(‘caller’) is method to generate a string into random address.
29.	 Uint160 caller = 1; address(1); is method to generate a random address from numerical value. Integer must be of type uint160 or bytes20.
30.	Vm.prank(caller) method is used to initiate txn with specific addres instead of msg.sender or address (this).. vm.deal(caller, 10 ether) is method to fund the random address for payable txn.
31.	 hoax(caller, 10 ether) combines both prank and deal...if not amount is given it allocate fund with value of 2^128.
32.	 Snapshot file is created only when test is passed. Snapshot gives gas usage stat, have same functionality as test and cli used similarly like –fork-url.
33.	 gasleft() is solidity method to check for gas consumption.
34.	 vm.txGasPrice(); is method to simulate vm gas price without which gas measurement can fail. It takes argument 1 shows accurate gas remaining. 2 will multiply remaining with 2.
35.	uint gasDiff = (gasStart - gasEnd) * tx.gasprice; gives out gas that remains after txn ends
